...
export class Participant2210vpController {
  constructor(
    ...
    private readonly gxParticipantController: Participant2210vpController
  ) {}
  ...
  async verifyParticipantVP(): // @Body(new JoiValidationPipe(VerifiablePresentationSchema))
  ...
  Promise<void /*ValidationResultDto*/> {
  ...
  private async verifySignedParticipantVP(typedVerifiablePresentation: TypedVerifiablePresentation): Promise<void /*ValidationResultDto*/> {
  ...
  private async validateSignedParticipantVC(participantVC: IVerifiableCredential) {
    const validationResult = null //: validationResultWithoutContent = await this.selfDescriptionService.validateVC(participantVC)
